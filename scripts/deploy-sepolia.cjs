...
async function main() {
  try {
    ...
    const assets = [
      { name: "AionicUSD", symbol: "AUSD", collateralFactor: 2000, liquidationFactor: 12500, etherAmount: "1.0", initialPrice: BigInt(100000000) },
      { name: "AionicEURO", symbol: "AEURO", collateralFactor: 3000, liquidationFactor: 12000, etherAmount: ".5", initialPrice: BigInt(108000000) },
      { name: "AionicYEN", symbol: "AJYN", collateralFactor: 4000, liquidationFactor: 11000, etherAmount: ".5", initialPrice: BigInt(650000) },
      { name: "AionicGPB", symbol: "AGPB", collateralFactor: 5000, liquidationFactor: 13000, etherAmount: ".5", initialPrice: BigInt(130000000) },
      { name: "AionicYAUN", symbol: "ACNY", collateralFactor: 4000, liquidationFactor: 11000, etherAmount: ".5", initialPrice: BigInt(14000000) },
    ];
    ...
    await simulatePriceChange(dataFeeds, "AJYN", BigInt(950000));
    await simulatePriceChange(dataFeeds, "AEURO", BigInt(110000000));


    const Asset = await hre.ethers.getContractFactory("Asset");
    const asset1 = Asset.attach(assetAddresses["AJYN"]);
